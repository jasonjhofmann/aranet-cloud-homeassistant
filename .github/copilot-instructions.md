...
> Note: This is a **custom integration** (HACS), not part of HA Core. Some HA-Core-internal commands (`python -m script.hassfest`, `python -m script.translations`, `requirements_all.txt`) do not apply here. Use the GitHub Actions equivalents shown below.
...
- **MyPy on integration**: `mypy custom_components/<integration_domain>`
...
- **HA fixtures for custom integrations**: Use the `pytest-homeassistant-custom-component` package — it provides `MockConfigEntry`, the `hass` fixture, and equivalents of HA Core's `tests.common`.

### Dependencies & Requirements
- Manage Python deps in `pyproject.toml` (dev + test extras) or `requirements.txt` / `requirements-dev.txt`.
- Install with `uv pip install -e .[dev]` or `uv pip install -r requirements-dev.txt`.
- Runtime requirements pinned in `manifest.json` `"requirements"` field — what HA installs at integration load time.

### Translations
- After modifying `strings.json`, ensure `translations/en.json` matches (manual mirror is fine for English-only integrations).
- For multi-language translations, follow HA's [translations workflow](https://developers.home-assistant.io/docs/internationalization/custom_integration).

### Project Validation
- **Hassfest + HACS validation** runs in CI via GitHub Actions, not locally. Recommended `.github/workflows/validate.yml`:
  ```yaml
  jobs:
    validate-hassfest:
...
def mock_device_api() -> Generator[MagicMock]:
    ...
    with patch("custom_components.my_integration.MyDeviceAPI", autospec=True) as api_mock:
...
async def init_integration(
    ...
    with patch("custom_components.my_integration.PLATFORMS", platforms):
...
### Validation Commands
```bash
# Hassfest + HACS validation runs in CI via GitHub Actions (.github/workflows/validate.yml).
# There's no local equivalent for custom integrations — let CI catch manifest issues.

# Check quality_scale.yaml against current rules — review manually against:
